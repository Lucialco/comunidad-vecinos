...
const ALL_LINKS = [
  { href: '/', label: 'Inicio', roles: ['public', 'admin', 'presidente'], icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
  { href: '/admin', label: 'Administrador', roles: ['admin', 'presidente'], icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z' },
  { href: '/presidente', label: 'Presidente', roles: ['presidente'], icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
]
...
export default function Navbar() {
  ...
  if (status === 'loading') return (
    <header className="bg-white border-b border-stone-200 sticky top-0 z-40 shadow-sm h-16" />
  )
          ...
          {session?.user ? (
            <div className="flex items-center gap-2 ml-2 pl-2 border-l border-stone-200">
              <div className="text-right hidden md:block">
                ...
                <p className="text-xs text-[#C9A227] leading-none mt-0.5 capitalize">{session.user.role}</p>
              </div>
              <button
            ...
            <Link href="/login"
              className="ml-2 flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg bg-[#C9A227] text-white hover:bg-[#A07D1A] transition-colors">
              Acceder
            </Link>
          )}
        </div>
        ...
        <div className="sm:hidden border-t border-stone-100 bg-white shadow-lg">
          ...
          {session?.user ? (
            <div className="px-4 py-3 border-t border-stone-100">
              <p className="text-xs text-stone-500 mb-1">{session.user.name} · <span className="text-[#C9A227] capitalize">{session.user.role}</span></p>
              <button
            ...
            <Link href="/login" onClick={() => setOpen(false)}
              className="flex items-center gap-3 px-4 py-3.5 text-sm font-medium text-[#C9A227] hover:bg-[#FBF3DA]">
              Acceder
            </Link>
          )}
        </div>
